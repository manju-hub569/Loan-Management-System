								...
								if(data.status == false){
									...
									$.each(data, function(key, value){
										$("#load-table").append("<tr>"
																...
																+"<td>" + value.total + "</td>"
																+"<td>" + value.status + "</td>"+
																"<td><button class='edit-btn' data-eid='"+value.name+"'>Edit Status</button></td>"+
																"<td><button class='delete-btn' data-id='"+value.name+"'>Delete</button></td>"+
...
$("#save-btn").on("click",function(e){
	...
	$.ajax({
		...
		success : function(data){
			...
			$("addform").trigger("reset");
				...
				$(document).on("click",".delete-btn", function(){
					var custnam = $(this).data("id");
					var obd = {cname : custnam},
					 myJSN = JSON.stringify(obd);
					//console.log(myJSN);
					$.ajax({
						url:'http://localhost/loan_system/api_delete.php',
							type:"POST",
							data:myJSN,
							success : function(data){
								loadTable();
		...
		<ul style = "margin:5px 0px;" id = "toggle_ul">
			<li><a href = "cust_detail.php">customer data</a></li>
		</ul>
	</div>				
	</body>
</html>